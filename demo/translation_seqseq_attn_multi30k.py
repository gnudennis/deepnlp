...
from core.utils.visual import show_heatmaps


def args(training=True):
    parser = argparse.ArgumentParser(description="train and save your model")

    parser.add_argument('--arch', default='translation_seqseq_attn', help='architecture')
    ...
    if training:
        ...
        parser.add_argument('--saved-pth', default='translation_seqseq_attn.pth', help='path for the trained model')
    else:
        parser.add_argument('--weights-pth', default='translation_seqseq_attn.pth', help='net weights')
...
def train(args: argparse.Namespace):
    ...
    nw = 0
    vocabs, loaders = translation.utils.dataset.load_data_multi30k(
        args.batch_size,
        num_workers=nw,
...
def evaluate(args: argparse.Namespace):
    ...
    for i, (src, trg) in enumerate(test_data):
        ...
        attention_weights = (torch.cat([x[0] for x in attention_weight_seq], dim=1))[:, :, 1:].unsqueeze(0)
